...
## 1. Objective
Build the "Brain" of the fuzzer. My goal was to create a modular logic engine (`analyzer.py`) that can take raw IAM JSON and mathematically prove if it is dangerous, then connect it to the live collector (`collector.py`).
...
* **Method:** Used `isinstance()` to check types. If the input is a String or Dict, I immediately wrap it in a List `[]`. (See [line 18 of my code](https://github.com/Oluwatobi-Mustapha/iam-fuzzer/blob/main/src/analyzer.py)).
...
Result Screenshot from my terminal: ![alt text](<Wk2 - IAM Fuzzer.jpeg>)